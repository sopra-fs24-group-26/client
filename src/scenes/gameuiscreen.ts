...
export class GameUiScreen extends Phaser.Scene {
    ...
    public create(): void {
        ...
        this.displayDrawnTiles();
        isMyTurn ? this.setAllTilesInteractive() : this.setAllTileNotInteractive();
    }
    ...
    public displayDrawnTiles(): void {
        ...
        for (let i: int = 0; i < nrTiles; i++) {
            ...
            tile.setInteractive();
            this.displayedTiles.push([tile, myTiles[i].id.toString(), myTiles[i].type]);
    ...
    private checkAdjacency(uiCoordinates: int[]): boolean {
        ...
        const adjacencyMap: Nullable<AdjacencyMap> = TileManager.getAdjacencyMap();
        assert(adjacencyMap);
        return adjacencyMap.isAdjacent(gameWorldCoordinates[0], gameWorldCoordinates[1]);
    }
    ...
    private checkConnections(uiCoordinates: int[]): boolean {
        ...
        const adjacencyMap: Nullable<AdjacencyMap> = TileManager.getAdjacencyMap();
        assert(adjacencyMap);
        return adjacencyMap.isAligned(gameWorldCoordinates[0], gameWorldCoordinates[1], this.currentTile);
    }
